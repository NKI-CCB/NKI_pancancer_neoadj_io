...
def run_snakemake(yaml_config,config):
    ...
    # run snakemake
    os.system('snakemake --use-conda --conda-frontend conda --use-singularity --singularity-args "-B ' + bind_d1 + ' -B ' + bind_d2  +  ' -B ' +  bind_d3 + ' -B ' +  bind_d4 + ' -B ' + bind_d5 + ' " --cores ' + str(config["cores"]) + ' ' + str(yaml_config["snakemake"]))
