...
params:
  output_dir: "gene_signatures_meta_excl_clin_response"
...
combined_dir <- here(data_dir, "gene_signatures_excl_clin_response")
...
signature_response_ssgsea_bellini <- read_csv2(
  here(data_dir, "gene_signatures_bellini_excl_clin_response",
       "signature_response_ssgsea.csv")) %>%
  mutate(Trial = "BELLINI") 
 
signature_response_ssgsea_imcision <- read_csv2(
  here(data_dir, "gene_signatures_imcision_excl_clin_response",
       "signature_response_ssgsea.csv")) %>%
...
signature_response_ssgsea_niche_mss <- read_csv2(
  here(data_dir, "gene_signatures_niche_mss_excl_clin_response",
       "signature_response_ssgsea.csv")) %>%
  mutate(Trial = "NICHE-MSS") 

signature_response_ssgsea_opacin_neo <- read_csv2(
  here(data_dir, "gene_signatures_opacin_neo_excl_clin_response",
       "signature_response_ssgsea.csv")) %>%
...
signature_response_ssgsea_matisse <- read_csv2(
  here(data_dir, "gene_signatures_matisse_excl_clin_response",
       "signature_response_ssgsea.csv")) %>%
...
# signature_response_ssgsea_excl_dmmr$signature %>% unique()
...
table(gene_signatures_meta_analysis$Signature %in% signature_response_c$signature)
gene_signatures_meta_analysis$Signature[!gene_signatures_meta_analysis$Signature %in% signature_response_c$signature]
```
# ----------
...
# Lint script
```{r}
# lint("~/NKI_pancancer_neoadj_io/src/gene_signatures_meta_excl_clin_response.Rmd.Rmd")
lint(filename = "~/NKI_pancancer_neoadj_io/src/gene_signatures_meta_excl_clin_response.Rmd",
