...
# BiocManager::install("GSVA", version = "2.2")
packageVersion("GSVA")

# Downgrade matrixStats, compatibility with GSVA
# remotes::install_version("matrixStats", version="1.1.0") 
# Got solved after GSVA 2.2 and Bioconductor 3.2, but needs R 4.5
set.seed(42)
select <- dplyr::select
rename <- dplyr::rename
here <- here::here
...
# tpm_counts <- read_csv2(here("data", "processed", "rnaseq_data",
#                              "rnaseq_tpm.csv"))
# tpm_counts <- tpm_counts %>% column_to_rownames("gene")
...
gene_signatures %>% filter(is.na(Ref)) %>% select(Signature) %>% unique()

# gene_signatures %>% select(-c(Gene, Description)) %>% unique() %>% view()
```
...
for (current_gene_signature in names(gene_signatures_l)) {
  ...
  signature_rpm_data_paired <- signature_rpm_data %>%
    filter(Patient %in% paired_samples$Patient)
...
# ----------
# Bagaev Clustering on ssGSEA scores
```{r}
bagaev_signatures <- gene_signatures %>% filter(Ref == "Bagaev et al. 2021")

bagaev_ssgsea <- ssgsea_results_df %>%
  select(all_of(c("CF_sample", unique(bagaev_signatures$Signature))))

bagaev_ssgsea_m <- bagaev_ssgsea %>%
  column_to_rownames("CF_sample") %>%
  mutate_if(is.character, as.numeric) %>%
  as.matrix() %>%
  t()
matrix_colnames <- colnames(bagaev_ssgsea_m)
bagaev_ssgsea_m <- t(apply(
  bagaev_ssgsea_m, 1, function(x) scale(x, center = TRUE, scale = TRUE)))

colnames(bagaev_ssgsea_m) <- matrix_colnames
...
mat_breaks <- quantile_breaks(bagaev_ssgsea_m, n = 11)

bagaev_heatmap <- pheatmap::pheatmap(bagaev_ssgsea_m,
  annotation_col = annot_df,
...
bagaev_heatmap_pre <- pheatmap::pheatmap(bagaev_ssgsea_m[, rownames(annot_df)],
  annotation_col = annot_df,
  col = paletteer_d(palette = "RColorBrewer::RdYlBu",
                    n = length(mat_breaks) - 1, direction = -1),
  show_colnames = FALSE,
