...
params:
  ...
  response_definition: "Original" # Original MPR pCR
---

Author comment: File description comment: The purpose of this script is to perform WES analysis on parsed WES files
...
# Parse params and setup output folder
...
combined_maf_df_samples <- combined_maf_df %>%
  ...
  filter(Tumor_Sample_Barcode %in% dna_metadata_f$Tumor_Sample_Barcode)
...
combined_maf_df %>% filter(FILTER == "common_variant")
...
combined_maf_dp20_vaf_df <- read_csv2(
  here(wes_dir, "combined_maf_dp20_vaf_df.csv"))
...
n_variants_dp20_vaf_tmb <- read_csv2(
  here(output_dir, "n_variants_dp20_vaf_tmb.csv"))

tmb_response_rate <- read_csv2(here(output_dir, "tmb_response_rate.csv"))
...
maftools_BP_SMG_pathways %>%
  filter(Pathway == "Wnt/B-catenin signaling") %>% pull(Gene) %>% unique()

oncoKB_cancerGeneList %>%
  filter(Hugo.Symbol %in% c("PTCH1", "CTNND1", "LZTR1", "NDP", "CHD4", "CHD8"))
...
top_genes_n_samples %>%
  filter(Gene %in% c("TP53" , "KRAS", "NRAS", "HRAS", "BRAF", "BRCA1",
                     "BRCA2", "CDK2NA", "B2M"))
top_genes_n_samples %>%
  filter(Gene %in% c("TP53" , "KRAS", "NRAS", "HRAS", "BRAF", "BRCA1",
                     "BRCA2", "CDK2NA", "B2M")) %>% view()
...
pathway_level_mutation_df <- pathway_level_mutation_df %>%
  left_join(
    n_variants_dp20_vaf_tmb %>%
      select(c("Tumor_Sample_Barcode", colnames(n_variants_dp20_vaf_tmb)[
...
n_variants_dp20_vaf_tmb_status %>%
  write_csv2(here(output_dir, "n_variants_dp20_vaf_tmb_status.csv"))

n_variants_dp20_vaf_tmb_status <- read_csv2(
  here(output_dir, "n_variants_dp20_vaf_tmb_status.csv"))

# GIE AP Genetic Immune Evasion Antigen Presentation
# GIE IFNy Genetic Immune Evasion Antigen IFNy pathway
n_variants_dp20_vaf_tmb_status <- n_variants_dp20_vaf_tmb_status %>%
  mutate(GIE_IFNy = ifelse(as.logical(GIE_IFNy),
                           "GIE_IFNy_mut", "GIE_IFNy_wt")) %>%
  mutate(GIE_AP = ifelse(as.logical(GIE_AP),
                         "GIE_AP_mut", "GIE_AP_wt"))
```
...
pathway_level_mutation_m <- pathway_level_mutation_df %>%
  select(TMB, starts_with("SMG"),
         starts_with("GIE"),
         starts_with("SIGPW"), "DDR", "RAS_RAF", "BRCA1_2") %>%
  as.matrix() 
...
## Frequency in trials vs TMB

```{r, fig.height = 6, fig.width = 6}
 table(n_variants_dp20_vaf_tmb_status$Trial,
       n_variants_dp20_vaf_tmb_status$GIE_AP, useNA = "ifany")

 table(n_variants_dp20_vaf_tmb_status$Trial,
       n_variants_dp20_vaf_tmb_status$GIE_IFNy, useNA = "ifany")
 
table(ifelse(n_variants_dp20_vaf_tmb_status$TMB >= 10, "TMB-H", "TMB-L"),
      n_variants_dp20_vaf_tmb_status$GIE_AP, useNA = "ifany")
...
immune_hits_ap_freq <- table(pathway_level_mutation_df$Trial,
  ...
  mutate(immune_hits_ap = ifelse(as.logical(immune_hits_ap),
                                 "AP_mut", "AP_wt")) %>%
...
table(pathway_level_mutation_df$immune_hits_ap > 0,
  ...
  mutate(Total = sum(Count), Percentage = Count / Total * 100)  %>% 
  ggplot(aes(x = immune_hits_ap, y = Percentage, fill = Response)) +
  geom_col() +
  theme_Publication() +
  scale_fill_manual(values = c(Color_NR, Color_R))
...
immune_hits_ifny_freq <- table(
  ...
  mutate(immune_hits_ifny = ifelse(as.logical(immune_hits_ifny),
                                   "pIFNy_mut", "pIFNy_wt")) %>%
...
pathway_level_mutation_df_non_dmmr <- pathway_level_mutation_df %>%
  filter(!Trial %in% c("NICHE-MSI"))
...
table(specific_mutation_df$Trial,
      specific_mutation_df$Response,
      specific_mutation_df$POLE)
...
non_response_pathways <- read_csv(here("data", "processed",
                                       "PanCancer_pathway_mutations",
                                       "Non_responders",
                                       "Non_response_pathways__stats.csv"))
...
oncoplot_nr_pathways_map <- read_csv2(
  here(output_dir, "oncoplot_nr_pathways_map.csv"))

oncoplot_nr_pathways <- oncoplot_nr_pathways %>%
  left_join(oncoplot_nr_pathways_map)
oncoplot_nr_pathways %>%
  write_csv2(here(output_dir, "oncoplot_nr_pathways.csv"))
...
pathway_data_msigdb_combined <- read.csv2(
  here(output_dir, "pathway_proportions_msigdb",
       "pathway_data_combined.csv")) %>%
  left_join(trial_group_map)

pathway_gene_data_msigdb_combined <- read.csv2(
  here(output_dir, "pathway_proportions_msigdb",
       "pathway_gene_data_combined.csv")) %>%
  left_join(trial_group_map)
...
oncoplot(subsetMaf(combined_maf_specific_f, tsb = dna_metadata_f %>%
                     filter(Trial == "BELLINI") %>%
                     pull(Tumor_Sample_Barcode)), 
         clinicalFeatures = c("Response", "Trial"),
         pathways = "smgbp",
         selectedPathways = c("RNA_abundance", "MAPK_signaling",
                              "Genome_integrity"),
         annotationColor = annotation_color_list,
         topBarData = "TMB", logColBar = FALSE, sortByAnnotation = TRUE,
         removeNonMutated = FALSE)
...
oncoplot(subsetMaf(combined_maf_specific_f, tsb = dna_metadata_f %>%
                     filter(Trial == "MATISSE") %>% pull(Tumor_Sample_Barcode)), 
         clinicalFeatures = c("Response", "Trial"),
         pathways = "smgbp",
...
oncoplot(subsetMaf(combined_maf_specific_f, tsb = dna_metadata_f %>%
                     filter(Trial == "NABUCCO") %>% pull(Tumor_Sample_Barcode)), 
         clinicalFeatures = c("Response", "Trial"),
         pathways = "smgbp",
...
oncoplot(subsetMaf(combined_maf_specific_f, tsb = dna_metadata_f %>%
         ...
         clinicalFeatures = c("Response", "Trial"),
         pathways = "smgbp",
         selectedPathways = c("MAPK_signaling",
                              "Protein_homeostasis/ubiquitination",
                              "Genome_integrity"),
         annotationColor = annotation_color_list,
         topBarData = "TMB", logColBar = FALSE, sortByAnnotation = TRUE,
         removeNonMutated = FALSE)
...
oncoplot(subsetMaf(combined_maf_specific_f, tsb = dna_metadata_f %>%
                     filter(Trial == "MATISSE") %>%
                     pull(Tumor_Sample_Barcode)), 
         clinicalFeatures = c("Response", "Trial"),
         pathways = oncoplot_custom_pathways %>%
           filter(Pathway == "IFN_y_pathway"),
...
table(pathway_level_mutation_df$Trial,
      pathway_level_mutation_df$SIGPW_PI3K_signaling)
...
cytoband_pq_df <- read_csv2(here(output_dir, "cytoband_pq_df.csv"))

chr_arms_exclude <- c("chrYp", "chrYq", "chrXp", "chrXp", "chr13p",
                      "chr14p", "chr15p", "chr21p", "chr22p")

chromosome_sizes <- cytoband_pq_df %>%
  group_by(chr) %>%
...
facets_data %>%
  filter(!is.na(emflags)) %>%
  select(CF_sample_tumor, emflags) %>%
  unique() %>%
  factor_summary()
...
segment_status_n_map <- data.frame(
  segment_status = c("deep_del", "loss", "normal", "gain", "amp"),
  cn = c(0, 1, 2, 3, 4))
...
facets_c_arms_p <- facets_c_arms_p %>%
  filter(!chr_arm %in% c("chrYp", "chrYq", "chr13p",
                         "chr14p", "chr15p", "chr21p", "chr22p")) %>%
...
facets_c_cytobands %>% filter(is.na(chr_arm))
...
facets_c_cytobands %>% filter(chr_arm_cytoband == "chr9_p21.3") %>%
  select(cytoband, start, end, cn, type, cnlr.median) %>%
  factor_summary()
...
facets_c_cytobands_excl_dMMR_amp %>%
  write_csv2(here(output_dir, "facets_c_cytobands_excl_dMMR_amp.csv"))

facets_c_cytobands_excl_dMMR_amp_freq %>%
  write_csv2(here(output_dir, "facets_c_cytobands_excl_dMMR_amp_freq.csv"))
...
cytoband_fisher_amp_excl_dmmr %>%
  write_csv2(here(output_dir, "cytoband_fisher_amp_excl_dmmr.csv"))
```

```{r}
facets_c_cytobands_excl_dMMR_amp_freq[
  select_top_extremes(facets_c_cytobands_excl_dMMR_amp_freq$Delta_amp,
                      top_n = 10), ]

facets_c_cytobands_excl_dMMR_amp_freq[
  select_top_extremes(facets_c_cytobands_excl_dMMR_amp_freq$Delta_amp,
                      top_n = 10), ] %>%
  ggplot(aes(x = reorder(chr_arm_cytoband, Delta_amp), y = Delta_amp)) +
  geom_col() + coord_flip() + theme_Publication()

facets_c_cytobands_excl_dMMR_del_freq %>%
  filter(chr == "chr9") %>%
  ggplot(aes(x = reorder(chr_arm_cytoband, Delta_del_hemizyg),
             y = Delta_del_hemizyg)) + geom_col() + coord_flip() +
  theme_Publication()

facets_c_cytobands_excl_dMMR_amp_freq %>%
  filter(chr == "chr9") %>%
  ggplot(aes(x = reorder(chr_arm_cytoband, Delta_amp),
             y = Delta_del_hemizyg)) + geom_col() + coord_flip() +
  theme_Publication()
...
## Analyze HLA LOH
```{r}
facets_c_cytobands_HLA <- facets_c_cytobands %>%
  filter(chr_arm == "chr6p") %>%
  mutate(segment_size = end - start,
         segment_size_mb = segment_size / 1e6)

facets_c_cytobands_HLA_p21.3 <- facets_c_cytobands_HLA %>%
  filter(cytoband %in% c("p21.31", "p21.32", "p21.33")) 

facets_c_cytobands_HLA %>%
  ggplot(aes(x = start, y = cn, col = cytoband)) +
  geom_point() + theme_Publication() +
  geom_line(group = "CF_sample")

facets_c_cytobands_HLA_p21.3 %>%
  ggplot(aes(x = start, y = cn, col = cytoband)) +
  geom_point() + theme_Publication() +
  geom_line(group = "CF_sample")

facets_c_cytobands_HLA_p21.3  %>% factor_summary()

```

```{r, fig.height = 30, fig.width = 30}
facets_c_cytobands_HLA %>%
  mutate(cn = ifelse(cn >5, 5, cn)) %>%
  ggplot(aes(x = start, y = cn, col = cytoband)) +
  geom_point() + theme_Publication() +
  geom_line(group = "CF_sample") +
  facet_wrap(~CF_sample) +
  theme(axis.text.x = element_blank())
...
```{r}
n_distinct(facets_c_cytobands_HLA$CF_sample)

facets_c_cytobands_HLA %>%
  ggplot(aes(x = start, y = cn, col = cytoband)) +
  geom_point() + theme_Publication() + geom_line(group = "CF_sample")
...
```{r}
# GRCh38 coordinates for HLA class I genes
hla_coords <- data.frame(
  gene = c("HLA-A", "HLA-B", "HLA-C"),
  chr  = c("6", "6", "6"),
  start = c(29941260, 31353867, 31268749), 
  end   = c(29949572, 31367067, 31272130) 
) 

hla_coords

# Function to find overlapping segments
overlap_segments <- function(df, chr, start, end) {
  df %>%
    filter(
      chrom == chr,
      start <= end,
...
# For each HLA gene, extract overlapping FACETS segments
hla_segments <- lapply(1:nrow(hla_coords), function(i) {
  gene <- hla_coords$gene[i]
  chr  <- hla_coords$chr[i]
  s    <- hla_coords$start[i]
  e    <- hla_coords$end[i]
...
# Check consistency of CN and type across cytobands per sample + gene
HLA_consistency_check <- hla_segments %>%
  group_by(CF_sample, gene) %>%
  summarise(
    n_cytobands = n_distinct(cytoband),
    cn_values   = paste(sort(unique(cn)), collapse = ","),
    type_values = paste(sort(unique(type)), collapse = ","),
    HLA_gene_consistent_cn   = n_distinct(cn) == 1,
    HLA_gene_consistent_type = n_distinct(type) == 1,
    .groups = "drop"
  )

print(HLA_consistency_check)

HLA_consistency_check %>% factor_summary()
table(HLA_consistency_check$type_values, HLA_consistency_check$cn_values)

# CN breakpoint but still duplication
# Deep del or hemizygous, these will be grouped
HLA_consistency_check %>%
  filter(HLA_gene_consistent_cn == FALSE | HLA_gene_consistent_type == FALSE)

# For each sample, count distinct CN and type states on chr6p
chr6p_cytobands_summary <- facets_c_cytobands %>%
  filter(chr_arm == "chr6p") %>%
  group_by(CF_sample) %>%
  summarise(
    n_segments      = n(),
    n_cn_states     = n_distinct(cn),
    cn_states       = paste(sort(unique(cn)), collapse = ","),
    n_type_states   = n_distinct(type),
    type_states     = paste(sort(unique(type)), collapse = ","),
    .groups = "drop"
  ) %>%
  left_join(facets_data_metrics %>%
              mutate(CF_sample = CF_sample_tumor))

chr6p_cytobands_summary %>% factor_summary()

chr6p_summary <- facets_c_arms %>%
  filter(chr_arm == "chr6p") %>%
  group_by(CF_sample) %>%
  summarise(
    n_segments      = n(),
...
chr6p_summary %>% factor_summary()

chr6p_summary %>%
  write_csv2(here(output_dir, "chr6p_summary.csv"))

HLA_consistency_check %>%
  write_csv2(here(output_dir, "HLA_consistency_check.csv"))

HLA_consistency_check <- read_csv2(
  here(output_dir, "HLA_consistency_check.csv"))
```

```{r}
# HLA status flag
HLA_consistency_flag <- HLA_consistency_check %>%
    select(CF_sample, HLA_gene_consistent_type) %>%
    unique() %>%
  group_by(CF_sample) %>%
  summarize(HLA_gene_consistent_type = !any(
    HLA_gene_consistent_type == FALSE)) %>%
  unique()

HLA_LOH_df <- HLA_consistency_check %>%
  select(CF_sample, gene, type_values) %>%
  unique() %>%
  pivot_wider(names_from = "gene", values_from = type_values)%>%
  rename(HLA_A = "HLA-A", HLA_B = "HLA-B", HLA_C = "HLA-C") %>%
  mutate(HLA_consistent_type = HLA_A == HLA_B & HLA_B == HLA_C) %>%
  left_join(HLA_consistency_flag) %>%
  mutate(HLA_grouped = "Unassigned") %>%
  mutate(HLA_grouped = ifelse(HLA_A %in% c("LOH", "DUP-LOH") |
                                HLA_B %in% c("LOH", "DUP-LOH") |
                                HLA_C %in% c("LOH", "DUP-LOH") ,
                              "HLA-LOH", HLA_grouped)) %>%
  mutate(HLA_grouped = ifelse(HLA_A %in% c("DEL", "HEMIZYG") |
                                HLA_B %in% c("DEL", "HEMIZYG") |
                                HLA_C %in% c("DEL", "HEMIZYG") %in% c("DEL", "HEMIZYG"),
                              "HLA-DEL", HLA_grouped)) %>%
  mutate(HLA_grouped = ifelse(HLA_A %in% c("DUP", "NEUTR") &
                                HLA_B %in% c("DUP", "NEUTR") &
                                HLA_C %in% c("DUP", "NEUTR"),
                              "HLA-Intact", HLA_grouped))%>%
  mutate(HLA_Intact = ifelse(HLA_grouped %in% c("HLA-Intact"),
                             "HLA-Intact", "HLA-Altered")) %>%
  left_join(facets_data_metrics %>%
                    mutate(CF_sample = CF_sample_tumor))

HLA_LOH_df %>% filter(!HLA_consistent_type) %>% factor_summary()

HLA_LOH_df %>%
  select(Trial, CF_sample_tumor, HLA_grouped, HLA_Intact, HLA_A, HLA_B, HLA_C,
         HLA_consistent_type, HLA_gene_consistent_type) %>%
  view()

table(HLA_LOH_df$HLA_A, HLA_LOH_df$HLA_grouped)
table(HLA_LOH_df$HLA_B, HLA_LOH_df$HLA_grouped)
table(HLA_LOH_df$HLA_C, HLA_LOH_df$HLA_grouped)

table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_A)
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_A) %>%
  as.data.frame() %>%
  setNames(c("Trial", "HLA_A", "Freq")) %>%
  ggplot(aes(x = Trial, y = Freq, fill = HLA_A)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_B)
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_B) %>%
  as.data.frame() %>%
  setNames(c("Trial", "HLA_B", "Freq")) %>%
  ggplot(aes(x = Trial, y = Freq, fill = HLA_B)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_C)
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_C) %>%
  as.data.frame() %>%
  setNames(c("Trial", "HLA_C", "Freq")) %>%
  ggplot(aes(x = Trial, y = Freq, fill = HLA_C)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_grouped)
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_grouped) %>%
  prop.table(margin = 1) %>%
  round(2)
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_grouped) %>%
  as.data.frame() %>%
  setNames(c("Trial", "HLA_grouped", "Freq")) %>%
  ggplot(aes(x = Trial, y = Freq, fill = HLA_grouped)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_Intact)
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_Intact) %>%
  prop.table(margin = 1) %>%
  round(2)
...
table(HLA_LOH_df$Trial, HLA_LOH_df$HLA_Intact) %>%
  as.data.frame() %>%
  setNames(c("Trial", "HLA_Intact", "Freq")) %>%
  ggplot(aes(x = Trial, y = Freq, fill = HLA_Intact)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
HLA_LOH_df %>% write_csv2(here(output_dir, "HLA_LOH_df.csv"))
HLA_LOH_df <- read_csv2(here(output_dir, "HLA_LOH_df.csv"))

```

```{r}
HLA_LOH_TMB_df <- HLA_LOH_df %>%
  left_join(n_variants_dp20_vaf_tmb) %>%
  mutate(TMB_status = ifelse(TMB >= 10, "TMB-H", "TMB-L"))

table(HLA_LOH_TMB_df$HLA_A, HLA_LOH_TMB_df$TMB_status)
...
table(HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_A) %>%
  as.data.frame() %>%
  setNames(c("TMB_status", "HLA_A", "Freq")) %>%
  ggplot(aes(x = TMB_status, y = Freq, fill = HLA_A)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
table(HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_grouped)
table(HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_grouped) %>%
  prop.table(margin = 1) %>%
  round(2)
...
table(HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_grouped) %>%
  as.data.frame() %>%
  setNames(c("TMB_status", "HLA_grouped", "Freq")) %>%
  ggplot(aes(x = TMB_status, y = Freq, fill = HLA_grouped)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
table(HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_Intact)
table(HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_Intact) %>%
  prop.table(margin = 1) %>%
  round(2)
...
table(HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_Intact) %>%
  as.data.frame() %>%
  setNames(c("TMB_status", "HLA_Intact", "Freq")) %>%
  ggplot(aes(x = TMB_status, y = Freq, fill = HLA_Intact)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
```{r}
table(HLA_LOH_TMB_df$Trial,
      HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_grouped) %>%
  as.data.frame() %>%
  setNames(c("Trial", "TMB_status", "HLA_grouped", "Freq")) %>%
  ggplot(aes(x = Trial, y = Freq, fill = HLA_grouped)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + 
  facet_wrap(~TMB_status)
...
table(HLA_LOH_TMB_df$Trial,
    ...
    as.data.frame() %>%
    setNames(c("Trial", "TMB_status", "HLA_grouped", "Freq")) %>%
  pivot_wider(names_from = HLA_grouped, values_from = Freq) %>%
  mutate(Total = `HLA-DEL` + `HLA-Intact` + `HLA-LOH`,
         `HLA-DEL` = `HLA-DEL` / Total,
         `HLA-Intact` = `HLA-Intact` / Total,
         `HLA-LOH` = `HLA-LOH` / Total) %>%
  pivot_longer(cols = c(`HLA-DEL`, `HLA-Intact`, `HLA-LOH`),
                        names_to = "HLA_grouped", values_to = "Prop") %>%
  mutate(Trial_n = sprintf("%s (%s)", Trial, Total)) %>%
  ggplot(aes(x = Trial_n, y = Prop, fill = HLA_grouped)) +
  geom_col()  + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + 
  facet_wrap(~TMB_status, scales = "free")

table(HLA_LOH_TMB_df$Trial,
      HLA_LOH_TMB_df$TMB_status, HLA_LOH_TMB_df$HLA_Intact) %>%
  as.data.frame() %>%
  setNames(c("Trial", "TMB_status", "HLA_Intact", "Freq")) %>%
  ggplot(aes(x = Trial, y = Freq, fill = HLA_Intact)) +
  geom_col() + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + 
  facet_wrap(~TMB_status)
...
table(HLA_LOH_TMB_df$Trial,
      HLA_LOH_TMB_df$TMB_status,
      HLA_LOH_TMB_df$HLA_Intact) %>%
    as.data.frame() %>%
    setNames(c("Trial", "TMB_status", "HLA_Intact", "Freq")) %>%
  pivot_wider(names_from = HLA_Intact, values_from = Freq) %>%
  mutate(Total = `HLA-Altered` + `HLA-Intact`,
         `HLA-Altered` = `HLA-Altered` / Total,
         `HLA-Intact` = `HLA-Intact` / Total) %>%
  pivot_longer(cols = c(`HLA-Altered`, `HLA-Intact`),
                        names_to = "HLA_Intact", values_to = "Prop") %>%
  mutate(Trial_n = sprintf("%s (%s)", Trial, Total)) %>%
  ggplot(aes(x = Trial_n, y = Prop, fill = HLA_Intact)) +
  geom_col()  + theme_Publication()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + 
  facet_wrap(~TMB_status, scales = "free")
...
table(HLA_LOH_TMB_df$Trial,
       HLA_LOH_TMB_df$HLA_Intact,
      HLA_LOH_TMB_df$Response, useNA = "ifany") %>%
  as.data.frame() %>%
  setNames(c("Trial", "HLA_Intact", "Response", "Freq")) %>%
  ggplot(aes(x = Response, y = Freq, fill = Trial)) +
  geom_col() + theme_Publication() +
  scale_fill_manual(values = trial_colors) +
  facet_wrap(~HLA_Intact)
...
table(HLA_LOH_TMB_df$Trial,
  ...
  as.data.frame() %>%
  setNames(c("Trial", "HLA_Intact", "Response", "Freq")) %>% 
  filter(!HLA_Intact %in% c("HLA-Intact")) %>%
  ggplot(aes(x = Response, y = Freq, fill = Trial)) +
  geom_col() + theme_Publication() +
  scale_fill_manual(values = trial_colors) +
  facet_wrap(~HLA_Intact)
...
table(HLA_LOH_TMB_df$Trial,
       HLA_LOH_TMB_df$HLA_grouped,
      HLA_LOH_TMB_df$Response, useNA = "ifany") %>%
  as.data.frame() %>%
  setNames(c("Trial", "HLA_grouped", "Response", "Freq")) %>%
  ggplot(aes(x = Response, y = Freq, fill = Trial)) +
  geom_col() + theme_Publication() +
  scale_fill_manual(values = trial_colors) +
  facet_wrap(~HLA_grouped)
...
table(HLA_LOH_TMB_df$Trial,
  ...
  as.data.frame() %>%
  setNames(c("Trial", "HLA_grouped", "Response", "Freq")) %>%
  filter(!HLA_grouped %in% c("HLA-Intact")) %>%
  ggplot(aes(x = Response, y = Freq, fill = Trial)) +
  geom_col() + theme_Publication() +
  scale_fill_manual(values = trial_colors) +
  facet_wrap(~HLA_grouped)
...
table(HLA_LOH_TMB_df$Trial,
  ...
  setNames(c("Trial", "HLA_grouped", "Response", "Freq")) %>%
  filter(!HLA_grouped %in% c("HLA-Intact")) %>%
  ggplot(aes(x = HLA_grouped, y = Freq, fill = Trial)) +
  geom_col() + theme_Publication() +
  scale_fill_manual(values = trial_colors) +
  facet_wrap(~Response)
...
# Clonal TMB
```{r}
facets_data_df$emflags %>% table(useNA = "ifany")

facets_data_df %>%
  select(CF_sample_tumor, Trial, emflags) %>%
  unique() %>% view()

facets_data_df %>% head()

combined_maf_dp20_vaf_df %>% head()
table(combined_maf_dp20_vaf_df$Chromosome)
table(facets_data_df$chrom)
...
facets_data_metrics %>%
  ggplot(aes(x = emflags, y = Purity)) + geom_boxplot() +
  coord_flip() + theme_Publication()
```

## Join variant VAF and FACETs information

```{r}
# Safe helper to ensure all FACETS columns exist
fix_facets_cols <- function(df) {
  if (!is.data.frame(df)) {
    df <- as.data.frame(df)
  }
  required <- c("cf.em", "tcn.em", "lcn.em", "cnlr.median.clust", "mafR.clust", "type")
  missing  <- setdiff(required, names(df))
  df[missing] <- NA
  df
}

join_facets_variants <- function(variant_sub, facets_sub) {

  facets_sub <- fix_facets_cols(facets_sub)

  gr_var <- GRanges(
    seqnames = variant_sub$Chromosome,
    ranges   = IRanges(start = variant_sub$Start_Position,
                       end   = variant_sub$End_Position)
  )

  gr_fac <- GRanges(
    seqnames = facets_sub$chrom,
    ranges   = IRanges(start = facets_sub$start,
                       end   = facets_sub$end),
    cf.em  = facets_sub$cf.em,
    tcn.em = facets_sub$tcn.em,
    lcn.em = facets_sub$lcn.em,
    cnlr.median.clust = facets_sub$cnlr.median.clust,
    mafR.clust = facets_sub$mafR.clust,
    type = facets_sub$type
    
  )

  hits <- findOverlaps(gr_var, gr_fac)

  joined <- cbind(
    variant_sub[queryHits(hits), ],
    as.data.frame(mcols(gr_fac)[subjectHits(hits), ])
  )
...
# Split both dataframes by sample
variants_by_sample <- split(combined_maf_dp20_vaf_df, combined_maf_dp20_vaf_df$CF_sample_tumor, drop = FALSE)
facets_by_sample <- split(facets_data_df,
                          facets_data_df$CF_sample_tumor, drop = FALSE)

# Intersect sample names present in both
common_samples <- intersect(names(variants_by_sample), names(facets_by_sample))
length(common_samples)

common_samples[1:5]

# Run overlap join per sample
results_list <- map(common_samples, function(s) {
  join_facets_variants(variants_by_sample[[s]], facets_by_sample[[s]])
})

# Combine all samples into one final dataframe
variant_facets_cn_df <- bind_rows(results_list)

variant_facets_cn_df <- variant_facets_cn_df %>%
  left_join(facets_data_metrics) %>%
  left_join(patient_metadata_combined %>%
              select(Patient, Sex))

dim(variant_facets_cn_df)
dim(combined_maf_dp20_vaf_df)
nrow(variant_facets_cn_df) / nrow(combined_maf_dp20_vaf_df) * 100

# Inspect
table(variant_facets_cn_df$Chromosome)

variant_facets_cn_df %>%
  filter(is.na(Purity)) %>%
  ggplot(aes(x = VAF, fill = Trial, alpha = 0.5)) +
  geom_density() +
  scale_fill_manual(values = trial_colors) +
  theme_Publication()
...
variant_facets_cn_df %>%
  filter(Chromosome == "X") %>%
  select(Patient, Trial, Sex, Chromosome, tcn.em, type) %>%
  unique() %>%
  view()
...
facets_data_df %>%
  ...
  left_join(patient_metadata_combined %>%
              select(Patient, Sex)) %>%
  select(Patient, Trial, Sex, chrom, tcn.em, type) %>%
  unique() %>%
  view()

variant_facets_cn_df %>%
  write_csv2(here(output_dir, "variant_facets_cn_df.csv"))
```

## Impute purity from diploid median VAF

```{r}
# First examine all variants
vaf_summaries <- combined_maf_dp20_vaf_df %>%
  filter(Chromosome %in% c(1:22)) %>%
  group_by(CF_sample_tumor, Trial) %>%
...
vaf_summaries %>%
  ggplot(aes(x = mean_VAF, y = median_VAF, col = Variants)) +
  geom_point() + theme_Publication() +
  geom_abline(slope = 1 , intercept = 0)

vaf_summaries %>%
  ggplot(aes(x = mean_VAF, y = q3_VAF, col = Variants)) +
  geom_point() + theme_Publication() +
  geom_abline(slope = 1 , intercept = 0)

vaf_summaries %>%
  ggplot(aes(x = median_VAF, y = q3_VAF, col = Variants)) +
  geom_point() + theme_Publication() +
  geom_abline(slope = 1 , intercept = 0)

vaf_summaries %>%
  ggplot(aes(x = Variants, y = median_VAF, col = Trial)) +
  geom_point() + theme_Publication() +
  scale_color_manual(values = trial_colors)

vaf_summaries %>%
  ggplot(aes(x = Variants, y = mean_VAF, col = Trial)) +
  geom_point() + theme_Publication() +
  scale_color_manual(values = trial_colors)

vaf_summaries %>%
  ggplot(aes(x = Variants, y = q3_VAF, col = Trial)) +
  geom_point() + theme_Publication() +
  scale_color_manual(values = trial_colors)
...
# Merge summaries back into the main df for plotting
plot_df <- combined_maf_dp20_vaf_df %>%
  filter(Chromosome %in% c(1:22)) %>%
  left_join(vaf_summaries, by = "CF_sample_tumor")

# Plot: VAF distribution per sample with median + Q3
ggplot(plot_df, aes(x = CF_sample_tumor, y = VAF)) +
  geom_violin(fill = "grey85", color = "grey40", scale = "width") +
  geom_point(aes(y = median_VAF), color = "blue", size = 2) +
  geom_point(aes(y = q3_VAF), color = "red", size = 2) +
  labs(
    title = "VAF Distribution per Sample",
    subtitle = "Blue = median VAF, Red = Q3 VAF",
    x = "Sample",
...
```{r}
# Filter for balanced diploid copy number neutral, without LOH
variant_facets_cn_df %>%
  filter(tcn.em == 2) %>% 
  filter(Chromosome %in% c(1:22)) %>%
  select(Purity, Ploidy, type, tcn.em) %>%
  factor_summary() 

diploid_vaf_summaries <- variant_facets_cn_df %>%
  filter(tcn.em == 2) %>% 
  filter(type == "NEUTR") %>%
  filter(Chromosome %in% c(1:22)) %>%
  group_by(CF_sample_tumor, Trial) %>%
  summarize(diploid_VAF_Variants = n(),
            diploid_mean_VAF = mean(VAF),
            diploid_median_VAF = median(VAF),
            diploid_q3_VAF = quantile(VAF, 0.75, na.rm = TRUE)
            ) %>%
  ungroup() %>%
  mutate(Imputed_median_purity = 2 * diploid_median_VAF) %>%
  mutate(Imputed_median_purity = ifelse(Imputed_median_purity > 1, 1, Imputed_median_purity)) %>%
  mutate(Imputed_purity = 2 * diploid_q3_VAF) %>%
  mutate(Imputed_purity = ifelse(Imputed_purity > 1, 1, Imputed_purity)) 

dim(vaf_summaries)
dim(diploid_vaf_summaries)

vaf_summaries %>%
  left_join(diploid_vaf_summaries) %>%
  factor_summary()

vaf_summaries_c <- vaf_summaries %>%
  left_join(diploid_vaf_summaries) %>%
  left_join(facets_data_metrics) %>%
  mutate(Purity_flag = ifelse(
    (is.na(Purity) & ! is.na(Imputed_purity)),
    "median_diploid_VAF_Imputed_purity", "FACETS_purity")) %>%
  ungroup()

vaf_summaries_c %>%
  select(Trial, Purity, Imputed_purity, Purity_flag, emflags) %>%
  view()

vaf_summaries_c %>%
  select(-c(Source_MAF, Tumor_Sample_Barcode)) %>%
  filter(is.na(Purity)) %>%
  factor_summary()
...
ggplot(vaf_summaries_c,
       aes(x = Imputed_median_purity, y = Imputed_purity)) +
  geom_point(alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  labs(
    title = "Comparison of Median-VAF Purity vs Q3-VAF Purity",
    x = "Purity (2 × median VAF)",
    y = "Purity (2 × Q3 VAF)"
  ) +
  theme_Publication()

vaf_summaries_long <- vaf_summaries_c %>%
  select(CF_sample_tumor, Purity, Imputed_median_purity,
         Imputed_purity, emflags) %>%
  tidyr::pivot_longer(cols = starts_with("Imputed_"),
                      names_to = "method",
                      values_to = "Imputed") %>%
  mutate(Delta = Purity - Imputed) 

ggplot(vaf_summaries_long,
       aes(x = reorder(CF_sample_tumor, Imputed),
           y = Imputed, color = method)) +
  geom_point(alpha = 0.7) +
  labs(
    title = "Purity Estimates per Sample (Median vs Q3)",
    x = "Sample (sorted by Imputed purity)",
    y = "Purity estimate"
  )  +
  theme_Publication()  +
  theme(axis.text.x = element_blank())
...
ggplot(vaf_summaries_long,
       aes(x = reorder(CF_sample_tumor, Purity),
           y = Imputed, color = method)) +
  geom_point(alpha = 0.7) +
  labs(
    title = "Purity Estimates per Sample (Median vs Q3)",
    x = "Sample (sorted by FACETs purity)",
    y = "Purity estimate"
  ) +
  theme_Publication() +
  theme(axis.text.x = element_blank())
...
```{r}
vaf_summaries_long %>%
  filter(method == "Imputed_median_purity") %>%
  mutate(Delta_perc = Delta * 100) %>%
  select(-CF_sample_tumor) %>%
  factor_summary()

vaf_summaries_long %>%
  filter(method == "Imputed_purity") %>%
  mutate(Delta_perc = Delta * 100) %>%
  select(-CF_sample_tumor) %>%
  factor_summary()

vaf_summaries_long %>%
  ggplot(aes(x = Purity, y = Delta, col = method)) +
  geom_point(alpha = 0.7) +
  theme_Publication()
...
vaf_summaries_long %>%
  ...
  theme_Publication() +
  theme(axis.text.x = element_blank()) +
  facet_wrap(~emflags, scales = "free")

vaf_summaries_long %>%
  ggplot(aes(x = method, y = Delta)) +
  geom_boxplot() +
  theme_Publication() 
...
# Checking VAF distribution over all variants
combined_maf_dp20_vaf_df %>%
         filter(Chromosome %in% c(1:22)) %>% 
  left_join(vaf_summaries)%>% 
  filter(!is.na(Purity))  %>%
  ggplot(aes(x = VAF, fill = Trial)) +
  geom_density() +
  facet_wrap(~ CF_sample_tumor, scales = "free_y") +
  theme_bw() +
  theme(
  ...
  labs(
    title = "VAF Density per Sample",
    subtitle = "Blue = median VAF, Red = Q3 VAF, Green = 1/2 FACETs purity (if available)",
    x = "VAF",
    y = "Density"
  ) +
  geom_vline(aes(xintercept = median_VAF ), color = "blue") +
  geom_vline(aes(xintercept = q3_VAF), color = "red") +
  geom_vline(aes(xintercept = Purity / 2), color = "green") +
  scale_fill_manual(values = trial_colors)

# Only checking balanced diploid VAF distributions
variant_facets_cn_df %>%
  filter(tcn.em == 2) %>% 
  filter(type == "NEUTR") %>% 
  filter(!is.na(Purity)) %>% 
  left_join(vaf_summaries_c) %>%
  ggplot(aes(x = VAF, fill = Trial)) +
  geom_density() +
  facet_wrap(~ CF_sample_tumor, scales = "free_y") +
  theme_bw() +
  ...
  labs(
    title = "VAF Density per Sample (Balanced Diploid Regions)",
    subtitle = "Blue = median VAF, Red = Q3 VAF, Green = 1/2 FACETs purity (if available)",
    x = "VAF",
    y = "Density"
  ) +
  geom_vline(aes(xintercept = diploid_median_VAF), color = "blue") +
  geom_vline(aes(xintercept = diploid_q3_VAF), color = "red") +
  geom_vline(aes(xintercept = Purity / 2), color = "green") +
  scale_fill_manual(values = trial_colors)
```
...
variant_facets_cn_df %>%
  ...
  labs(
    title = "VAF Density per Sample (Balanced Diploid Regions)",
    subtitle = "Blue = median VAF, Red = Q3 VAF, Green = FACETs purity (if available)",
    x = "VAF",
    y = "Density"
  ) +
  geom_vline(aes(xintercept = diploid_median_VAF), color = "blue") +
...
```{r}
vaf_summaries_c %>%
  ggplot(aes(x = mean_VAF, y = diploid_mean_VAF)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 1, intercept = 0)

vaf_summaries_c %>%
  ggplot(aes(x = median_VAF, y = diploid_median_VAF)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 1, intercept = 0)
...
vaf_summaries_c %>%
  ggplot(aes(x = q3_VAF, y = diploid_q3_VAF)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 1, intercept = 0)
...
vaf_summaries_c %>%
  ...
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  facet_wrap(~ifelse(is.na(Purity), "Missing Purity", "Estimated Purity"))+
  geom_abline(slope = 1, intercept = 0)
...
vaf_summaries_c %>%
  ggplot(aes(x = Purity, y = diploid_mean_VAF)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors)+
...
vaf_summaries_c %>%
  ggplot(aes(x = Purity, y = diploid_median_VAF)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors)+
  geom_abline(slope = 0.5, intercept = 0)
...
vaf_summaries_c %>%
  ggplot(aes(x = Purity, y = diploid_q3_VAF)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 0.5, intercept = 0)
...
vaf_summaries_c %>%
  ggplot(aes(x = Purity, y = mean_VAF)) + geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors)+
  geom_abline(slope = 0.5, intercept = 0)

vaf_summaries_c %>%
  ggplot(aes(x = Purity, y = median_VAF)) + geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 0.5, intercept = 0)

vaf_summaries_c %>%
  ggplot(aes(x = Purity, y = q3_VAF)) + geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 0.5, intercept = 0)

vaf_summaries_c %>%
  ggplot(aes(x = Purity, y = Imputed_median_purity)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 1, intercept = 0)
...
vaf_summaries_c %>%
  ggplot(aes(x = Purity, y = Imputed_purity)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 1, intercept = 0)
...
## Imputed purity from diploid median VAF (non LOH)
```{r}
vaf_summaries_c %>%
  ggplot(aes(x = diploid_median_VAF, y = Imputed_purity)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
...
vaf_summaries_c %>%
  ggplot(aes(x = diploid_q3_VAF, y = Imputed_purity)) +
  geom_point(aes(col = Trial)) +
  stat_cor() + theme_Publication() +
  scale_color_manual(values  = trial_colors) +
  geom_abline(slope = 2, intercept = 0) +
...
vaf_summaries_c %>% write_csv2(here(output_dir, "vaf_summaries_c.csv"))
vaf_summaries_c <- read_csv2(here(output_dir, "vaf_summaries_c.csv"))

```

# Parse Pyclone input
```{r}
colnames(variant_facets_ccf_df) 

# Build PyClone-VI input table
pyclone_df <- variant_facets_ccf_df %>%
  mutate(
    sample_id = CF_sample_tumor,
    mutation_id = paste0(Chromosome, ":", Start_Position),
    ref_counts = t_ref_count,
    alt_counts = t_alt_count,
    normal_cn = 2,
    minor_cn = lcn.em,
    major_cn = tcn.em - lcn.em,
    tumour_content = ifelse(
    (is.na(Purity) & ! is.na(Imputed_purity)), Imputed_purity, Purity)
  ) %>%
  select(
    mutation_id,
    sample_id,
...
pyclone_df %>% filter(is.na(tumour_content))

# Create output directory
dir.create(here("data", "processed", "pyclone_input"))

# Export one TSV per sample
for (current_sample in unique(pyclone_df$sample_id)) {
  pyclone_df %>%
    filter(sample_id == current_sample) %>%
    write_tsv(here("data", "processed", "pyclone_input",
                   paste0(current_sample, ".tsv")))
}
...
vcf_files_full_path <- list.files(
  path="/shared/data/niche/Pancancer_DNA_preprocessed_Pedro/VCF_files_filtered",
  pattern = "pass.vcf", full.names = TRUE)
vcf_files <- list.files(
  path="/shared/data/niche/Pancancer_DNA_preprocessed_Pedro/VCF_files_filtered",
  pattern = "pass.vcf", full.names = FALSE)
...
fit_res <- readRDS(here(output_dir, "fit_res_cosmic_v2_GRCh38.rds"))
...
colnames(Relative_contri_Perc_t) <- gsub("_", ".",
                                         colnames(Relative_contri_Perc_t))
Relative_contri_Perc_t <- Relative_contri_Perc_t %>%
  rename(name_vcf = name.vcf)
...
write.table(Relative_contri_Perc_t,
            here(output_dir, "Muts_Contribution_Perc_cosmic.txt"),
            sep="\t",quote=F, row.names = T, col.names = T)
write.table(fit_res_t,
            file = here(output_dir, "Muts_Contribution_cosmic.txt"),
            sep="\t", quote=F)
...
cosmic_combined_p %>%
  write_csv2(here(output_dir, "cosmic_combined_p.csv"))
cosmic_perc_combined_p %>%
  write_csv2(here(output_dir, "cosmic_perc_combined_p.csv"))
```
...
facets_data_metrics <- read_csv2(
  here(output_dir, "facets_data_metrics.csv"))
...
cosmic_combined_p <- read_csv2(here(output_dir, "cosmic_combined_p.csv"))
cosmic_perc_combined_p <- read_csv2(
  here(output_dir, "cosmic_perc_combined_p.csv"))

# Parsed in separate script for neoantigens and pyclone output
...
wes_features_df <- wes_features_df %>%
  left_join(pvactools_filtered_neoantigen_counts) %>%
  left_join(pyclone_vi_clonal_summary_df) %>%
  left_join(HLA_LOH_df) %>%
  left_join(vaf_summaries_c %>%
    select(all_of(
      c("CF_sample_tumor",
        colnames(vaf_summaries_c)[!colnames(vaf_summaries_c) %in%
...
wes_features_df %>% write_csv2(here(output_dir, "wes_features_df.csv"))
...
```{r}
wes_features_df %>%
  ggplot(aes(x = TMB, y = pvactools_filtered_neoantigen_load)) +
  geom_point() + stat_cor(method = "spearman") + theme_Publication()
...
# Additional HLA TMB Trial Checks

```{r}
table(wes_features_df$HLA_grouped, wes_features_df$`HLA-A`, useNA = "ifany")
table(wes_features_df$HLA_grouped, wes_features_df$`HLA-B`, useNA = "ifany")
table(wes_features_df$HLA_grouped, wes_features_df$`HLA-C`, useNA = "ifany")

table(wes_features_df$Trial, wes_features_df$HLA_grouped, useNA = "ifany")
table(wes_features_df$Trial, wes_features_df$HLA_grouped, useNA = "ifany")
table(wes_features_df$Trial, wes_features_df$HLA_grouped, useNA = "ifany")

table(wes_features_df$Trial, wes_features_df$HLA_grouped,
      wes_features_df$`HLA-A`, useNA = "ifany")
table(wes_features_df$Trial, wes_features_df$HLA_grouped,
      wes_features_df$`HLA-B`, useNA = "ifany")
table(wes_features_df$Trial, wes_features_df$HLA_grouped,
      wes_features_df$`HLA-C`, useNA = "ifany")
```

```{r}
table(wes_features_df$HLA_Intact, wes_features_df$HLA_alteration)

table(wes_features_df$HLA_Intact)
table(wes_features_df$HLA_Intact) %>% prop.table() %>% round(2)

table(wes_features_df$Trial, wes_features_df$HLA_Intact)
table(wes_features_df$Trial, wes_features_df$HLA_Intact) %>%
  prop.table(margin = 1) %>%
  round(2)

table(wes_features_df$TMB > 10,
      wes_features_df$HLA_Intact) %>%
  fisher.test()
```
...
```{r, fig.height = 10, fig.width = 10}
wes_features_df %>%
  filter(HLA_Intact == "HLA-Intact") %>%
  ggplot(aes(x = Response, y = TMB, fill = Response)) +
  facet_wrap(~Trial, scales = "free") +
  geom_boxplot() + theme_Publication() + stat_compare_means(method = "wilcox") +
  geom_hline(yintercept = 10, linetype = "dashed") +
  scale_fill_manual(values = c(Color_NR, Color_R)) +
  labs(title = "HLA-Intact Subsets")

wes_features_df %>%
  filter(HLA_Intact == "HLA-Altered") %>%
  ggplot(aes(x = Response, y = TMB, fill = Response)) +
  facet_wrap(~Trial, scales = "free") +
  geom_hline(yintercept = 10, linetype = "dashed") +
  geom_boxplot() + theme_Publication() + stat_compare_means(method = "wilcox") +
  scale_fill_manual(values = c(Color_NR, Color_R))+
  scale_fill_manual(values = c(Color_NR, Color_R)) +
  labs(title = "HLA-Altered Subsets")

wes_features_df %>%
  ggplot(aes(x = HLA_Intact, y = TMB)) + facet_wrap(~Trial, scales = "free") +
  geom_boxplot() + theme_Publication() + stat_compare_means(method = "wilcox") +
  geom_hline(yintercept = 10, linetype = "dashed")
```
...
wes_features_df %>%
  ggplot(aes(x = Response, y = TMB, fill = Response)) + facet_wrap(~HLA_Intact) +
  geom_boxplot() + theme_Publication() + stat_compare_means(method = "wilcox") +
  scale_fill_manual(values = c(Color_NR, Color_R))
...
```{r}
wes_features_df_matisse <- wes_features_df %>%
  filter(Trial == "MATISSE")

wes_features_df_matisse %>%
  ggplot(aes(x = HLA_Intact, y = TMB)) + facet_wrap(~Response) +
  geom_boxplot() + theme_Publication() +
  stat_compare_means(method = "wilcox") +
  labs(title = "MATISSE cSCC")
...
table(wes_features_df_matisse$HLA_Intact,
      wes_features_df_matisse$Response)
...
```{r}
wes_features_df %>%
  ggplot(aes(x = TMB, y = pvactools_filtered_neoantigen_load)) + geom_point()
```

```{r}
wes_features_df$pvactools_filtered_neoantigen_load %>% summary()
wes_features_df$pyclone_clonal_TMB %>% summary()
wes_features_df$Purity %>% summary()
wes_features_df %>% select(Purity_flag) %>% factor_summary()

wes_features_df %>%
  filter(Trial == "NICHE-MSI") %>%
  write_csv2(here("NICHE2_dMMR_wes_features_df.csv"))
```
