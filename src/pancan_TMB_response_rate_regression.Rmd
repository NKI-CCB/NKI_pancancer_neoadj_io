...
# Fit constrained model - previous Yarchoan slope
```{r}
# Fit with percentage
# model <- lm(Response_rate * 100 ~ log(median_TMB), data = tmb_response_rate)
model
...
# Constrained model: slope fixed at 10.8
model_constrained <- lm(Response_rate * 100 ~ offset(10.8 * log(median_TMB)), data = tmb_response_rate)
summary(model_constrained)
model_constrained
coef(model_constrained) %>% round(1)

# Compare models
anova(model, model_constrained)
```
