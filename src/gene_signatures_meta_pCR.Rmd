...
combined_dir <- here(data_dir, "gene_signatures_pCR")
combined_dir_excl_dmmr <- here(data_dir, "gene_signatures_pCR_excl_dmmr")
...
signature_response_ssgsea_bellini <- read_csv2(
  here(data_dir, "gene_signatures_pCR_BELLINI",
       "signature_response_ssgsea.csv")) %>%
  mutate(Trial = "BELLINI") 
 
# signature_response_ssgsea_imcision <- read_csv2(
#   here(data_dir, "gene_signatures_pCR_IMCISION",
#        "signature_response_ssgsea.csv")) %>%
#   mutate(Trial = "IMCISION") 

signature_response_ssgsea_nabucco <- read_csv2(
  here(data_dir, "gene_signatures_pCR_NABUCCO",
       "signature_response_ssgsea.csv")) %>%
  mutate(Trial = "NABUCCO") 

signature_response_ssgsea_niche_mss <- read_csv2(
  here(data_dir, "gene_signatures_pCR_NICHE-MSS",
       "signature_response_ssgsea.csv")) %>%
  mutate(Trial = "NICHE-MSS") 

signature_response_ssgsea_niche_msi <- read_csv2(
  here(data_dir, "gene_signatures_pCR_NICHE-MSI",
       "signature_response_ssgsea.csv")) %>%
  mutate(Trial = "NICHE-MSI")

signature_response_ssgsea_opacin_neo <- read_csv2(
  here(data_dir, "gene_signatures_pCR_OpACIN_neo",
       "signature_response_ssgsea.csv")) %>%
  mutate(Trial = "OpACIN_neo") 

signature_response_ssgsea_prado <- read_csv2(
  here(data_dir, "gene_signatures_pCR_PRADO",
       "signature_response_ssgsea.csv")) %>%
  mutate(Trial = "PRADO") 

signature_response_ssgsea_matisse <- read_csv2(
  here(data_dir, "gene_signatures_pCR_MATISSE",
       "signature_response_ssgsea.csv")) %>%
...
signature_response_ssgsea_c <- signature_response_ssgsea_bellini %>%
  ...
  rbind(signature_response_ssgsea_niche_msi) %>%
...
signature_response_rpm_expression_niche_msi <- read_csv2(
  here(data_dir, "gene_signatures_pCR_NICHE-MSI",
       "signature_response_rpm_expression.csv")) %>%
  mutate(Trial = "NICHE-MSI") 
...
signature_response_rpm_expression_c <- signature_response_rpm_expression_bellini %>%
  ...
  rbind(signature_response_rpm_expression_niche_msi) %>%
...
meta_studies_results_rna_all %>%
  ...
  ggplot(aes(
    x = factor(studlab, levels = c("Combined", "BELLINI",
                                   "MATISSE", "NABUCCO",
                                   "NICHE-MSS", "NICHE-MSI",
                                   "OpACIN_neo", "PRADO")), 
    y = factor(
...
signature_response_c %>%
  ...
  ggplot(aes(x = factor(Trial,
                        c("Combined", "BELLINI", "MATISSE",
                          "NABUCCO", "NICHE-MSS", "NICHE-MSI",
                          "OpACIN_neo", "PRADO")),
             y = factor(signature,
...
for (current_signature in signatures_to_plot) {
  ...
  plot <- signature_rpm_data_df %>% ggplot(aes_string(x = "Trial",
                                   y = current_signature,
                                   fill = "Response")) +
    geom_boxplot() +
    theme_Publication()  +
...
# Lint script
```{r}
# lint("~/NKI_pancancer_neoadj_io/src/gene_signatures_meta_pCR.Rmd")
lint(filename = "~/NKI_pancancer_neoadj_io/src/gene_signatures_meta_pCR.Rmd",
